...
function init() {
    ...
    var wireMaterial = new THREE.MeshBasicMaterial({
        color: 0xff0000, wireframe: false, transparent: true, opacity: 0
    });
    ...
    if (controlsEnabled) {
        ...
        for (i = 0; i < nenemy; i++) {
            ...
            if (window['directionenemy' + i] == 1) {
                window['enemy' + i].position.x += 20 * delta;
                window['enemy' + i].position.z += 0 * delta;
            ...
            if (window['directionenemy' + i] == 3) {
                window['enemy' + i].position.z += 20 * delta;
                window['enemy' + i].position.x += 0 * delta;
